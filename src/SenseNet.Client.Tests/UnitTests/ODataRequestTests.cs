...
public class ODataRequestTests
{
    ...
    [TestMethod]
    public void UploadRequest_NoParent()
    {
        ...
        Assert.AreEqual(
            "Invalid request properties: either ParentId or ParentPath must be provided.",
            exception.Message);
    }
    [TestMethod]
    public void UploadRequest_ParentId()
    {
        var request = new UploadRequest { ParentId = 42 };
        Assert.AreEqual($"{_baseUri}/OData.svc/content(42)/Upload?metadata=no",
            request.ToODataRequest(null).ToString());
    }
    [TestMethod]
    public void UploadRequest_ParentPath()
    {
        var request = new UploadRequest { ParentPath = "/Root/Content/MyFiles"};
        Assert.AreEqual($"{_baseUri}/OData.svc/Root/Content('MyFiles')/Upload?metadata=no",
            request.ToODataRequest(null).ToString());
    }
    [TestMethod]
    public void UploadRequest_ParentIdAndPath_PathIgnored()
    {
        var request = new UploadRequest { ParentId = 42, ParentPath = "/Root/Content/MyFiles" };
        Assert.AreEqual($"{_baseUri}/OData.svc/content(42)/Upload?metadata=no",
            request.ToODataRequest(null).ToString());
    }
